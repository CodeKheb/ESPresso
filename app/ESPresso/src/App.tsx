...
function App() {
  ...
  async function upsertProfile(db: Database, profile: Profile) {
    await db.execute(
      `INSERT INTO profiles (name, role, bio) VALUES ($1, $2, $3)
       ON CONFLICT(name) DO UPDATE SET role = excluded.role, bio = excluded.bio`,
  ...
  async function addContact(db: Database, person: Profile) {
      await db.execute(
          'INSERT INTO contacts (name, role, bio) VALUES ($1, $2, $3) ON CONFLICT(name) DO UPDATE SET role = excluded.role, bio = excluded.bio',
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      dbRef.current = db;
      const [ cachedProfiles, cachedContacts] = await Promise.all([
          fetchProfiles(db),
          fetchContacts(db),
      ]);
      if (!cancelled) {
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      ws.onmessage = async (json) => {
          ...
          if (msg.type === "profiles" && dbRef.current) {
  ...
  async function submitProfile() {
      if (!name || !role) return;
      const profile: Profile = { name, role, bio };
  ...
  onAddContanct={handleAddContact}
